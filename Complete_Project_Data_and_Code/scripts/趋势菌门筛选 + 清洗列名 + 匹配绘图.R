...
phylum_df <- read.csv(file.path(data_input_dir, "断根 phylum(1).csv"), header = TRUE, stringsAsFactors = FALSE, row.names = 1)

# 必须预先执行过：phylum_df 已加载并包含 Ratio 列
# 若未加载，请添加：
# phylum_df <- read.csv(file.path(data_input_dir, "断根 phylum(1).csv"), header = TRUE, row.names = 1)
...
# ---- Exudate trend plots (optional) ----
# exudate_df：行为样本，列为分泌物类别（若未预先加载，默认读取 data/input/冲积物分泌物全.csv）
if (!exists("exudate_df")) {
  exudate_path <- file.path(data_input_dir, "冲积物分泌物全.csv")
  if (file.exists(exudate_path)) {
    exudate_df <- read.csv(exudate_path, check.names = TRUE)
  } else {
    message("Skip exudate trend plots: exudate_df not found and input file missing: ", exudate_path)
  }
}
